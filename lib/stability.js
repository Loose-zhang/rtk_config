    ...
    if (reached) {
      ...
      try {
        ...
        if (!duplicate) {
          ...
          // 数据库：异步写入；失败时 JSON 仍作为持久兜底，之后会自动补写。
          dbInsertStableResult(record).then((saved) => {
            if (!saved) {
              log('warn', `Stable result ${record.id} retained in JSON and pending database sync`);
            }
          }).catch((e3) => {
            log('warn', `Database save failed for ${record.id}; JSON fallback retained: ${e3.message}`);
          });
          // 追加至成功列表文本（便于下一次计算使用）
          try {
            ensureDirectories();
