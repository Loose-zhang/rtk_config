...
const {
  ...
  resolveGeneratedFile, mergeStableResults, filterBatchStationsByHistory
} = store;
const db = require('./db');
...
function registerRoutes(app) {
  ...
  app.post('/api/batch/run-txt', (req, res) => {
    try {
      // 新建批处理默认重新计算提交列表中的全部站点，与自动多轮的后续轮次保持一致。
      // 调用方仍可显式传入 skipFailed/skipSucceeded=true 来启用历史结果过滤；
      // “继续上次批量”则由 /api/batch/resume 单独负责，只跳过已经成功的站点。
      const { txtPath, txtContent, inpstr1Base, inpstr2, inpstr3, concurrency, skipFailed = false, skipSucceeded = false, rounds, roundIntervalMinutes } = req.body || {};
      ...
      try {
        ...
        list = filterBatchStationsByHistory(list, {
          failedResults: readFailedResults(),
          stableResults: readStableResults(),
          skipFailed,
          skipSucceeded
        });
  ...
  app.get('/api/download/:filename', (req, res) => {
    try {
      ...
      // 验证文件名（防止目录遍历攻击；resolveGeneratedFile 内部也会拦截）
      if (!store.isSafeGeneratedName(fileName)) {
        return res.status(400).json({
          success: false,
          message: '非法的文件名'
        });
  ...
  app.get('/api/configs', (req, res) => {
    try {
      ...
      // 新布局：generated/<设备号>/<设备号>.conf；同时兼容旧的平铺 .conf 文件
      const entries = fs.readdirSync(config.generatedDir, { withFileTypes: true });
      const confItems = []; // { name, dir }
      const seen = new Set();
      for (const ent of entries) {
        try {
          if (ent.isDirectory()) {
            const confName = `${ent.name}.conf`;
            const p = path.join(config.generatedDir, ent.name, confName);
            if (fs.existsSync(p)) {
              confItems.push({ name: confName, dir: path.join(config.generatedDir, ent.name) });
              seen.add(confName);
            }
          } else if (ent.name.toLowerCase().endsWith('.conf') && !seen.has(ent.name)) {
            confItems.push({ name: ent.name, dir: config.generatedDir });
          }
        } catch (e) {
          log('warn', `Skipping entry due to error: ${ent.name} - ${e.message}`);
        }
      }

      const files = confItems
        .map(({ name, dir }) => {
          try {
            const stationId = name.toLowerCase().endsWith('.conf') ? name.slice(0, -5) : name;
            ...
            const logFilePath = logCandidates.map(n => path.join(dir, n)).find(p => fs.existsSync(p));
            ...
            log('warn', `Skipping file due to error: ${name} - ${e.message}`);
            return null;
          }
        })
        .filter(Boolean)
  ...
  app.delete('/api/config/delete/:filename', (req, res) => {
    try {
      ...
      // 查找文件的全部存在位置（新旧布局可能同时存在，须全部删除）
      const filePaths = store.resolveGeneratedFileAll(fileName);
      ...
      log('info', `Deleted config file: ${fileName} (${filePaths.length} location(s))`);

      // 同名日志文件也删除（如果存在，新旧布局全部位置）
      const stationId = fileName.replace('.conf', '');
      const logFile = `${stationId}.log`;
      for (const logPath of store.resolveGeneratedFileAll(logFile)) {
        try {
          fs.unlinkSync(logPath);
          log('info', `Deleted related log file: ${logPath}`);
        } catch (e) {
      ...
      // 若设备文件夹已空则一并移除
      try {
        const dir = path.join(config.generatedDir, stationId);
  ...
  app.delete('/api/log/delete/:filename', (req, res) => {
    try {
      ...
      log('info', `Deleted log file: ${fileName} (${filePaths.length} location(s))`);
  ...
  // 获取所有稳定结果
  // 优先从数据库读取；JSON 是持久兜底副本。数据库缺记录时合并展示并尝试补写回库。
  app.get('/api/stable-results', async (req, res) => {
    try {
      const jsonResults = readStableResults();
      if (db.dbIsReady()) {
        try {
          const databaseResults = await db.dbGetStableResults();
          const databaseIds = new Set(databaseResults.map(r => r.id).filter(Boolean));
          const jsonIds = new Set(jsonResults.map(r => r.id).filter(Boolean));
          const pendingRecords = jsonResults.filter(r => r && r.id && !databaseIds.has(r.id));
          let syncResult = { attempted: 0, saved: 0, failed: 0 };
          if (pendingRecords.length > 0) {
            syncResult = await db.dbInsertStableResults(pendingRecords);
            if (syncResult.failed > 0) {
              log('warn', `Database backfill incomplete: ${syncResult.saved}/${syncResult.attempted} saved`);
            }
          }
          const results = mergeStableResults(databaseResults, jsonResults);
          // 数据库中独有的历史记录也补入 JSON，保证数据库后续不可用时兜底文件完整。
          if (databaseResults.some(r => r && r.id && !jsonIds.has(r.id))) {
          ...
          return res.json({
            ...
            source: pendingRecords.length > 0 ? 'database+json' : 'database',
            databaseSync: syncResult
          });
        } catch (e) {
          // 数据库运行中不可用时回退 JSON，保证页面可用
          log('warn', `Read stable results from DB failed, falling back to JSON: ${e.message}`);
        }
      }
      const results = jsonResults.map(r => {
  ...
  // 保存新的稳定结果
  app.post('/api/stable-results', async (req, res) => {
    try {
      ...
      if (writeStableResults(results)) {
        // 尝试写入数据库；失败时 JSON 已持久保存，后续读取/重启会补写。
        const databaseSaved = await db.dbInsertStableResult(newResult);
        log('info', `Saved stable result for station ${newResult.stationId}`);
        res.json({
          success: true,
          message: databaseSaved ? '稳定结果已保存' : '稳定结果已保存到 JSON，数据库待同步',
          id: newResult.id,
          databaseSaved,
          persistence: databaseSaved ? 'database+json' : 'json-pending-database'
        });
      } else {
        res.status(500).json({
          success: false,
  ...
  // 删除稳定结果（数据库与 JSON 同步删除）
  app.delete('/api/stable-results/:id', async (req, res) => {
    try {
      ...
      if (db.dbIsReady()) {
        try {
          dbDeleted = (await db.dbDeleteStableResult(id)) || 0;
        } catch (e) {
          log('warn', `dbDeleteStableResult failed: ${e.message}`);
        }
      }

      // JSON 文件同步删除（可能已被轮次清空，不存在不算错误）
      let results = readStableResults();
      const initialLength = results.length;
      results = results.filter(r => r.id !== id);
      const jsonDeleted = results.length !== initialLength;
      if (jsonDeleted) {
        writeStableResults(results);
      }
      ...
      log('info', `Deleted stable result: ${id} (db: ${dbDeleted}, json: ${jsonDeleted})`);
      res.json({
        success: true,
        message: '记录已删除'
      });
  ...
  app.post('/api/rtkrcv/start', (req, res) => {
    try {
      ...
      // 检查配置文件是否存在（设备文件夹优先，兼容旧平铺布局）
      if (!resolveGeneratedFile(configFile)) {
        return res.status(404).json({
          success: false,
          message: '配置文件不存在'
        });
  ...
  app.get('/api/rtkrcv/log/:logFile', async (req, res) => {
    try {
      ...
      const logPath = resolveGeneratedFile(logFile);
